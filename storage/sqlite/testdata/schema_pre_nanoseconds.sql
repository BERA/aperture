...
-- stood at commit 5dc4854, before timestamps became integer nanoseconds: every
-- created_at/updated_at is TEXT (RFC3339Nano) and the audit log's instant is
-- still named ts_nanos. The startup guard's test builds a real database from
-- this file so it is proven against a genuine pre-change database rather than a
-- synthetic marker. Never edit it to track schema.sql -- it is a historical
-- record, and updating it would delete the very shape it exists to reproduce.
