-- FIXTURE, NOT A LIVE SCHEMA. This is storage/sqlite/schema.sql exactly as it
-- shipped in v0.5.0, before the apt_ table prefix and before timestamps became
-- integer nanoseconds: every table is unprefixed (accounts, grants, audit_log),
-- every created_at/updated_at is TEXT, and the audit log's instant is ts_nanos.
-- The startup guard's test builds a real database from this file, so "a v0.5.0
-- database is refused" is proven against the actual released schema. Never edit
-- it to track schema.sql -- it is a historical record, and updating it would
-- delete the very shape it exists to reproduce.
