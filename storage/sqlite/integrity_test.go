...
//
// Its mirror image is storage/memory/sentinel_columns_test.go. The two seed the
...
func TestDeletingASubjectAGrantNamesIsRefused(t *testing.T) {
	...
	for _, tc := range []struct {
	...
	}{
		{
			...
			del: func(s *Store) error { return s.DeletePrincipal(ctx, "alice") },
		},
		{
			...
			del: func(s *Store) error { return s.DeleteRole(ctx, "r-admin") },
		},
		{
			name: "group", kind: model.SubjectGroup, id: "eng", table: "apt_groups",
			free: func(t *testing.T, s *Store) {},
			del:  func(s *Store) error { return s.DeleteGroup(ctx, "eng") },
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
			s := freshStore(t)
